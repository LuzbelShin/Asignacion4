...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fun compruebaPeso(): Boolean{
            return peso.text.toString() != ""
        }

        fun compruebaEstatura(): Boolean{
            return estatura.text.toString() != ""
        }

        fun calculaIMC() {
            if(compruebaPeso() && compruebaEstatura()) {
                val pesoAux: Float = peso.text.toString().toFloat()
                val altura: Float = estatura.text.toString().toFloat()
                val estaturaAux = altura * altura
                val imcAux = pesoAux / estaturaAux

                imc.setText(imcAux.toString())
                rango.setText(calculaRango(imcAux))
            } else{
                imc.setText("Introduce valores validos por favor")
            }
        }
